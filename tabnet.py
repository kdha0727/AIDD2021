...
def inference(path, model, **kwargs):
    ...
    output_pred_labels = model.predict(data)
...
def preproc_data(data, label=None, train=True, val_ratio=0.2, seed=1234):
    if train:
        ...
        # SELECT_COLS = ['ALP', 'TG', 'BMI', 'LDL', 'ALT', 'HDL', 'BUN', 'age', 'Alb', 'SBP', 'AST', 'Cr', 'PR', 'FBG',
        #                'DBP', 'TC', 'HbA1c', 'CrCl', 'GGT', 'gender_enc']
        # X = X[SELECT_COLS]
...
def _fetch(y_true, y_score):
    preds_mapper = { idx : class_name for idx, class_name in enumerate(clf.classes_)}
    y_pred = np.vectorize(preds_mapper.get)(np.argmax(y_score, axis=1))
...
class F1Score(Metric):
    ...
    def __call__(self, y_true, y_score):
        y_true, y_pred = _fetch(y_true, y_score)
...
if __name__ == '__main__':
    ...
    tabnet_params = {"cat_idxs": [19],
                     "cat_dims": [19],  # 13 17 21
                     "cat_emb_dim": 2,
                     ...
                     #                  "scheduler_params":{
                     #                      "step_size":100, # how to use learning rate scheduler
                     #                      "gamma":0.9
                     #                  },
    ...
    if config.mode == 'train':
        ...
        clf.fit(
            X_train=dataset['all'].tensors[0].numpy(), y_train=dataset['all'].tensors[1].view(-1).numpy(),
            eval_set=[
                (dataset['val'].tensors[0].numpy(), dataset['val'].tensors[1].view(-1).numpy()),
            ],
            eval_name=['val'],
            eval_metric=['logloss', Precision, Recall, F1Score, ],  # 'accuracy', 'balanced_accuracy', 'auc',
            max_epochs=0, patience=1,
            batch_size=config.batch_size * 8, virtual_batch_size=config.batch_size,
            num_workers=0,
            weights=0,
            drop_last=False,
        )
